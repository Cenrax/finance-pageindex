...
def get_pdf_name(pdf_path):
    ...
    if isinstance(pdf_path, str):
        ...
        # Default case if pdf_path is neither a string nor BytesIO
        pdf_name = 'Untitled'
    return pdf_name
    ...
    if isinstance(data, list):
        for i in range(len(data)):
            if isinstance(data[i], dict) and 'physical_index' in data[i]:
                phys = data[i]['physical_index']
                if isinstance(phys, str):
                    if phys.startswith('<physical_index_'):
                        try:
                            idx = int(phys.split('_')[-1].rstrip('>').strip())
                        except (ValueError, TypeError):
                            data[i]['physical_index'] = None
                        else:
                            data[i]['physical_index'] = idx
                    elif phys.startswith('physical_index_'):
                        try:
                            idx = int(phys.split('_')[-1].strip())
                        except (ValueError, TypeError):
                            data[i]['physical_index'] = None
                        else:
                            data[i]['physical_index'] = idx
        ...
        phys = data
        if phys.startswith('<physical_index_'):
            content = phys.split('_')[-1].rstrip('>').strip()
            try:
                return int(content)
            except (ValueError, TypeError):
                return None
        elif phys.startswith('physical_index_'):
            content = phys.split('_')[-1].strip()
            try:
                return int(content)
            except (ValueError, TypeError):
                return None
